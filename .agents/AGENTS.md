...
- **Utilisateurs Standards** : Les rôles d'employés réguliers (`EMPLOYE`, `CONSULTANT`, `STAGIAIRE`, `PRESTATAIRE`) **ne doivent jamais** avoir accès aux données de géolocalisation (elles doivent être censurées/mises à `null` dans l'API et masquées dans l'interface utilisateur).
