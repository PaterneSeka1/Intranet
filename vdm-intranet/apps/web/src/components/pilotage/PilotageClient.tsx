...
export function PilotageClient({ role }: Props) {
  ...
  useEffect(() => {
    Promise.all([pilotageApi.summary(), pilotageApi.presenceByBu()])
      ...
      .catch((err) => {
        toast.error(err instanceof Error ? err.message : 'Impossible de charger le résumé.')
      })
  ...
  useEffect(() => {
    ...
    Promise.all([pilotageApi.connectionsChart(chartDays), pilotageApi.activityChart(chartDays)])
      ...
      .catch((err) => {
        toast.error(err instanceof Error ? err.message : 'Impossible de charger les graphiques.')
      })
  ...
  const fetchLog = useCallback(async (page: number, search: string, action: string) => {
    setLogLoading(true)
    try {
      const result = await pilotageApi.activityLog({
        page,
        limit: 25,
      ...
      toast.error(
        err instanceof Error ? err.message : "Impossible de charger le journal d'activité."
      )
    } finally {
      setLogLoading(false)
    }
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setLogPage(1)
      fetchLog(1, logSearch, logAction)
    }, 400)
    ...
    <div className="space-y-8">
      ...
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4">
          ...
          />
          <KpiCard
            label="Mandats du jour"
            value={summary.mandatesToday}
      ...
      {/* ── Graphiques ── */}
      <div
        className={`grid grid-cols-1 lg:grid-cols-2 gap-6 transition-opacity duration-300 ${chartLoading ? 'opacity-40 pointer-events-none' : ''}`}
      >
      ...
      {activityChart.length > 0 && (
        <div
          className={`bg-white rounded-2xl border border-gray-100 p-5 transition-opacity duration-300 ${chartLoading ? 'opacity-40 pointer-events-none' : ''}`}
        >
          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">
            {chartDays} derniers jours
          </p>
          ...
          <table className="w-full text-xs min-w-[700px]">
            <thead>
              <tr className="border-b border-gray-100">
                {['Date / Heure', 'Utilisateur', 'Rôle', 'Action', 'Entité', 'IP'].map((h) => (
                  <th
                    key={h}
                    className="text-left px-3 py-2.5 text-[10px] font-bold text-gray-400 uppercase tracking-wider whitespace-nowrap"
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody className={`transition-opacity duration-200 ${logLoading ? 'opacity-40' : ''}`}>
              {activityLog?.logs.map((log, idx) => (
                <tr
                  key={log.id}
                  className={`hover:bg-[#F28C38]/5 transition-colors ${idx % 2 === 0 ? '' : 'bg-gray-50/50'}`}
                >
                  ...
                  <td className="px-3 py-2.5 text-gray-400 whitespace-nowrap text-[11px]">
                    {log.user.role}
                  </td>
