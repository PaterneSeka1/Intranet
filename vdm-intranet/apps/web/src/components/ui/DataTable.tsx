                ...
                {columns.map(col => (
                  ...
                  >
                    <span className="inline-flex items-center gap-1.5">
                      {col.label}
                      {col.sortable && <SortIndicator dir={sort?.key === col.key ? sort.dir : null} />}
                    </span>
                  </th>
                ))}
                {hasActions && (
                  <th className={`${headPad} text-right text-[11px] font-bold text-gray-400 uppercase tracking-wider`}>
                    Actions
                  </th>
                )}
              </tr>
                ...
                >
                  {columns.map(col => (
                    ...
                    >
                      {col.render
                        ? col.render(row)
                        : (String((row as Record<string, unknown>)[col.key] ?? '') || <span className="text-gray-300">—</span>)}
                    </td>
                  ))}
                  {hasActions && (
          ...
          <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-2 px-4 py-2.5 border-t border-gray-50 bg-gray-50/40">
            ...
            <div className="flex items-center gap-3 text-xs text-gray-400 flex-wrap">
              ...
              <span className="hidden sm:flex items-center gap-1.5">
                ...
                {pageSizes.map(s => (
                  <button
                    key={s}
                    onClick={() => { setPageSize(s); goToPage(1) }}
            ...
            {totalPages > 1 && (
              <nav className="flex items-center gap-0.5" aria-label="Pagination">
                <NavBtn label="«" title="Première page" onClick={() => goToPage(1)} disabled={safePage <= 1} />
                <NavBtn label="‹" title="Page précédente" onClick={() => goToPage(Math.max(1, safePage - 1))} disabled={safePage <= 1} />
                ...
                <NavBtn label="›" title="Page suivante" onClick={() => goToPage(Math.min(totalPages, safePage + 1))} disabled={safePage >= totalPages} />
                <NavBtn label="»" title="Dernière page" onClick={() => goToPage(totalPages)} disabled={safePage >= totalPages} />
              </nav>
            )}
          </div>
        )}
...
function NavBtn({
  label, title, onClick, disabled = false, active = false,
}: { label: string; title?: string; onClick: () => void; disabled?: boolean; active?: boolean }) {
  return (
    <button
      title={title}
      onClick={onClick}
    ...
    <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-2 pt-3 border-t border-gray-50">
      ...
      {totalPages > 1 && (
        <nav className="flex items-center gap-0.5">
          ...
          <NavBtn label="‹" onClick={() => onPageChange(page - 1)} disabled={page <= 1 || loading} />
          {pages.map((p, i) =>
            p === '…' ? (
              <span key={`e${i}`} className="w-8 text-center text-xs text-gray-300">…</span>
            ) : (
              <NavBtn key={p} label={String(p)} onClick={() => onPageChange(p)} active={p === page} disabled={loading} />
            ),
          )}
          <NavBtn label="›" onClick={() => onPageChange(page + 1)} disabled={page >= totalPages || loading} />
          <NavBtn label="»" onClick={() => onPageChange(totalPages)} disabled={page >= totalPages || loading} />
        </nav>
      )}
    </div>
  )
