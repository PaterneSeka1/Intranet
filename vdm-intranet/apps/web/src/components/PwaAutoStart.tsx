...
function macosScript(appUrl: string, appName: string): string {
  ...
  const lines = [
    ...
    '',
    "# Le dossier où Chrome range les PWA installées s'appelle « Chrome Apps »",
    "# à l'affichage dans le Finder, mais son vrai nom sur le disque est",
    '# « Chrome Apps.localized » : le suffixe « .localized » est un mécanisme',
    "# macOS qui traduit uniquement l'affichage (ex. « Applications Chrome » en",
    '# français) sans renommer le dossier réel — un test bash voit toujours',
    '# le nom réel, jamais le nom traduit. Chercher le bundle par son nom',
    '# plutôt que de supposer le chemin du dossier parent évite ce piège, quelle',
    '# que soit la langue du Mac (même bug que le Menu Démarrer Windows, réglé',
    '# différemment côté Windows car le nom change vraiment là-bas).',
    `APP_PATH="$(find "$HOME/Applications" -maxdepth 2 -type d -iname '${safeName}.app' 2>/dev/null | head -n 1)"`,
    '',
    'if [ -z "$APP_PATH" ]; then',
