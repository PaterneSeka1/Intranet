...
- Demande : un employé en congé approuvé (app externe VEDEM/CONGE, `/Users/macbookpro/VEDEM/CONGE`, Next.js/Prisma/MongoDB séparée de ce repo) ne doit plus être marqué "Absent" dans VdM Intranet ; ajouter un widget "Employés en congé" visible par toute l'entreprise.
- Décisions actées avec l'utilisateur avant implémentation : rapprochement d'identité par `matricule` (Congé) == `username` (login VdM Intranet), email en repli ; widget placé sur `/accueil` uniquement (visible par tous les rôles, y compris accueil) ; autorisation explicite de modifier le repo VEDEM/CONGE.
...
- Demande : corriger Pilotage et Rapports pour qu'ils s'adaptent à chaque utilisateur — dette explicitement documentée comme "hors périmètre" dans le correctif "Absence marquée trop tôt / hors jour de travail" du même jour (`getPeriodReport` gardait son motif global Lundi-Vendredi au lieu du motif `workingDays` par employé).
- **`pilotage.service.ts::getPeriodReport`** (rapport hebdo/mensuel par BU) : restructuré pour calculer jour par jour, utilisateur par utilisateur — motif de travail propre (`workingDays`), jour férié (nouvelle méthode `public-holidays.service.ts::getHolidaysInRange`, un seul aller-retour DB pour toute la plage) et mandat explicite (toujours prioritaire, même hors motif/férié), même règle que `getSummary`/`getPresenceByBu`. Un jour hors périmètre de l'utilisateur (repos) n'est plus compté ni dans son total ni en "absent" — auparavant, un profil Pôle TV/Radio travaillant Mardi-Samedi était compté absent le lundi (jamais son jour) et son samedi travaillé n'était jamais évalué. Le champ `workingDays` par BU redevient un décompte de jours distincts effectivement travaillés par au moins un utilisateur de la BU (sa sémantique globale précédente n'avait plus de sens dès que les motifs diffèrent par employé).
- **`reports.service.ts::getGeneralData`/`generalCsv`** ("Rapport général" CSV) et **`reports-pdf.service.ts::generalHtml`** (même rapport en PDF) : le fallback `pres ? statut : 'Absent'` — qui ignorait totalement motif de travail, mandat, congé et heure attendue — est remplacé par le même calcul de statut que `/pilotage`/`/presences` (présence enregistrée > congé actif > mandat/motif de travail > heure attendue dépassée), avec les libellés `Repos`/`En attente`/`En congé` déjà établis ailleurs. Calculé une seule fois dans `getGeneralData`, consommé identiquement par le CSV et le PDF (pas de logique dupliquée entre les deux formats). Nouvelles dépendances injectées dans `ReportsService` : `PresenceScheduleService`, `PublicHolidaysService`, `LeaveSyncService` — `reports.module.ts` importe désormais `PresenceModule`/`PublicHolidaysModule`/`LeavesModule`, même principe que `PilotageModule`.
- Aucun changement de contrat API/frontend (types `PeriodReport`/`PeriodReportBu` et colonnes CSV/PDF inchangés), aucune migration Prisma requise (tous les champs utilisés existaient déjà), aucune régression sur les permissions (`assertAllowed`/`buildUserWhere` non touchés).
- **Non fait (hors périmètre explicite)** : `getPeriodReport` ne recalcule pas "pas encore arrivé ≠ absent" pour le jour courant s'il est inclus dans la plage en cours — limitation préexistante, distincte de l'adaptation par utilisateur demandée ici.
- Validation : `npx tsc --noEmit` (api + web) OK, `npx jest` (`apps/api`) 48/48 OK (aucune régression), `npm run build:api` OK, `rm -rf apps/web/.next` puis `npm run build:web` OK, `npx prettier --check` sur les 5 fichiers modifiés OK.
- `TACHE.md` mis à jour avec le détail complet de ce correctif.

### Nouvelle fonctionnalité — Synthèse par personne (absences, retards, minutes) dans le rapport de présences (2026-08-06)

- Demande : le rapport doit contenir le nombre total d'absences par personne, le nombre de jours de retard et le nombre de minutes de retard par personne. Clarifié avec l'utilisateur (choix explicite parmi plusieurs options) : ajouter cette synthèse **en plus** du détail jour par jour déjà présent dans le "Rapport de présences" (CSV+PDF), plutôt que de le remplacer ou de créer un rapport séparé.
- **`reports.service.ts::getPresenceSummaryRows`** (nouvelle méthode) : calcule par personne, sur la période demandée, `absences`/`lateDays`/`lateMinutesTotal`. Point d'attention : l'absence n'est jamais persistée en DB (seuls `PRESENT`/`LATE` le sont) — recalculée jour par jour avec exactement la même règle que `getSummary`/`getPeriodReport` du même jour (motif `workingDays` propre à l'utilisateur, mandat explicite toujours prioritaire, jour férié, congé actif exclu), y compris "pas encore arrivé ≠ absent" pour le jour courant ; un jour strictement passé sans présence est lui directement absent.
- `presenceCsv` (CSV) et `presencePdf`/`presenceHtml` (`reports-pdf.service.ts`, PDF) ajoutent ce tableau de synthèse par personne avant le détail journalier existant, sans le remplacer.
- Premier fichier de test du module `reports` : `reports.service.spec.ts` (10 tests) couvre retard/minutes cumulées, exclusion repos/congé/férié, mandat qui rend un jour hors motif comptable, et le cas "jour courant avant/après heure attendue" vs "jour strictement passé".
- Validation : `npx tsc --noEmit` (api + web) OK, `npx jest` (`apps/api`) 58/58 OK (48 préexistants + 10 nouveaux), `npm run build:api` OK, `rm -rf apps/web/.next` puis `npm run build:web` OK, `npx prettier --check` OK, `git diff --check` OK.
- `TACHE.md` mis à jour avec le détail complet de cette fonctionnalité.
...
- `presence.service.ts::canMandateUser` refuse désormais toute auto-cible (`requester.id === target.id`) pour tout rôle autre que `PDG`, vérifiée avant les règles de portée BU/Pôle existantes — s'applique donc à `CTO_ADMIN`, `DAF`, `RESPONSABLE_BU` et `RESPONSABLE_POLE`. `deleteMandate` ferme aussi le repli `createdById` pour ce cas (même principe que la règle CTO_ADMIN/PDG déjà en place depuis la session précédente du même jour).
- Frontend : `presences/planning/page.tsx` et `MandatesManager.tsx` retirent l'utilisateur courant des sélecteurs d'employé mandatable (sauf s'il est PDG), et masquent le bouton "Supprimer" sur ses propres mandats — anticipation côté UI de la règle backend, pour éviter un aller-retour en erreur 403.
- 5 nouveaux tests Jest (`presence.service.spec.ts`) : create/bulk-create/delete refusés pour un responsable ciblant lui-même, autorisés pour le PDG se ciblant lui-même — 48/48 tests OK au total (`apps/api`).
- Validation : `npx tsc --noEmit` (api + web) OK.
- `TACHE.md` mis à jour avec le détail de cet ajustement.

### Nouvelle demande réalisée — Rapports : export Excel natif, refonte visuelle PDF & période sur le Rapport général (2026-08-06)

- Demande : améliorer la mise en page des fichiers exportés et du rapport PDF, et pouvoir choisir une période d'exportation. Clarifié avec l'utilisateur (3 questions explicites) : ajouter la période au Rapport général (jusqu'ici figé sur "aujourd'hui") ; refonte visuelle complète des PDF (pas un simple ajustement) ; passage des CSV en Excel natif `.xlsx` (pas un CSV amélioré).
- **`reports.service.ts`** : extraction du calcul d'assiduité de `getPresenceSummaryRows` dans une méthode privée partagée `computeAttendanceSummaries`, réutilisée par `getGeneralData`, qui accepte désormais `dateFrom`/`dateTo` (défaut : aujourd'hui, comportement inchangé si absents) et renvoie en plus la synthèse absences/retards par utilisateur sur la période choisie ; les connexions sont comptées sur cette période plutôt que sur le seul jour courant (`connectionsCount` remplace `connectionsToday`).
- **`reports-excel.service.ts`** (nouveau) : remplace la construction CSV (`toCsv`, `presenceCsv`/`activityCsv`/`connectionsCsv`/`generalCsv`, retirés de `reports.service.ts`) par de vrais classeurs `.xlsx` (dépendance `exceljs`) — bandeau avec logo Veilleur des Médias, titre et période, en-tête de tableau coloré, volet gelé + filtre automatique, lignes zébrées, alignement numérique à droite, note de pied de page.
- **`reports-pdf.service.ts`** : refonte visuelle complète du gabarit commun (`renderShell`) — bandeau logo + titre + période, cartes de statistiques clés par rapport (absences, retards, connexions…), sections structurées, tableau avec en-tête répété à chaque page, pied de page avec numérotation et mention "usage interne".
- **`reports.controller.ts`** : les 4 routes CSV (`/reports/presence`, `/activity`, `/connections`, `/general`) renvoient désormais un `.xlsx` (`Content-Type` Excel) ; `/reports/general` et `/reports/general/pdf` acceptent maintenant `from`/`to`, comme les 3 autres rapports.
- Frontend : `PilotageClient.tsx` — le Rapport général a désormais un sélecteur de dates (comme les 3 autres), boutons renommés "Excel", noms de fichiers en `.xlsx`. `lib/csv-export.ts` renommé en `lib/excel-export.ts` (`downloadExcelBlob`).
- Logo intégré en base64 dans `apps/api/src/reports/assets/logo.ts` (PNG redimensionné en amont) — évite de dépendre d'un fichier statique copié au build, `nest build` ne copiant pas les assets non-TS par défaut.
- **Effet de bord corrigé** : l'ajout d'`exceljs` a révélé un conflit de types `Buffer` entre deux versions de `@types/node` imbriquées dans l'arbre de dépendances (`@fast-csv/format`/`@fast-csv/parse`, dépendances transitives d'`exceljs`, embarquaient chacune leur propre `@types/node@14`). Corrigé par un `overrides` dans le `package.json` racine (`"@types/node": "^22.0.0"`) et une réinstallation complète (`node_modules`/`package-lock.json` régénérés). Un cast `as any` isolé reste nécessaire sur l'appel `workbook.addImage` (incompatibilité résiduelle entre TypeScript 5.5 et le `Buffer` générique de `@types/node` 22, propre aux `.d.ts` d'`exceljs`, sans impact runtime).
- Aucun changement de permissions ni de migration Prisma requis.
- Validation : `npx jest` (`apps/api`) 58/58 OK (aucune régression), `npm run type-check --workspace=apps/api` OK, `npm run build:api` OK, `npm run type-check --workspace=apps/web` OK, `npm run build:web` OK.
- Non fait : vérification visuelle réelle du rendu Excel/PDF dans un tableur/lecteur PDF (même limite que les sessions précédentes — pas d'outil de rendu visuel disponible dans cet environnement) ; seule la génération sans erreur et la structure du contenu ont été vérifiées.
- `TACHE.md` mis à jour avec le détail complet de cette demande.
...
### Nouvelle demande réalisée — Blocage à la première connexion / changement de mot de passe obligatoire (2026-08-20)

- Demande : la logique de première connexion + changement de mot de passe obligatoire bloque les utilisateurs.
- **Cause racine** : régression introduite le 2026-07-29 (cf. entrée « "Mon historique" vide » ci-dessus). Ce jour-là, `LoginClient.tsx::handleSubmit` avait été modifié pour vérifier `requiresFirstLoginGeolocation` **avant** `mustChangePassword`, afin que la géolocalisation (`POST /presence/first-login`) s'exécute avant le changement de mot de passe forcé. Mais `jwt-auth.guard.ts` (ajouté le 2026-07-26, donc avant ce correctif) bloque déjà **toute** route API avec `403 Forbidden` dès que `mustChangePassword` est vrai, sauf celles listées dans `PASSWORD_CHANGE_ALLOWED_ROUTES` — et `POST /presence/first-login` n'y avait jamais été ajouté. Résultat : tout compte fraîchement créé/réinitialisé (`mustChangePassword = true` + aucune présence du jour) recevait un 403 sur l'appel de géolocalisation, restait bloqué sur `GeoLocationScreen.tsx` (seules actions possibles : "Réessayer", qui échoue à nouveau, ou "Se déconnecter"), et comme la présence du jour n'était jamais créée, le blocage persistait indéfiniment, y compris après déconnexion/reconnexion.
- **Correctif** : `apps/api/src/common/guards/jwt-auth.guard.ts` — ajout de `'POST /presence/first-login'` à `PASSWORD_CHANGE_ALLOWED_ROUTES`, pour rester cohérent avec le choix produit du 2026-07-29 (géolocalisation prioritaire sur le changement de mot de passe) sans y revenir.
- **Non fait** : `npm`/`node` indisponibles dans cet environnement (poste sans PATH configuré) — `npm run type-check --workspace=apps/api` et `npm run build:api` n'ont pas pu être lancés ici. Le changement est une addition d'une ligne dans une liste blanche existante (aucun impact de typage attendu), mais reste à valider par build/type-check et par test manuel réel (connexion avec un compte `mustChangePassword = true` sans présence du jour) sur un poste où l'environnement Node est disponible.

### Audit complet du dépôt & corrections — rôles, fiabilité backend, CI, accessibilité (2026-08-24)

Demande : analyse complète du dépôt (backend, frontend, rôles/permissions, infra) puis correction de l'ensemble des points relevés, partie par partie (roadmap et détail complet consignés dans `TACHE.md`). Environnement sans Docker/PostgreSQL disponible : aucune commande `db:*` lancée, tout le travail est du code pur sans nouvelle migration Prisma.

**Rôles enrichis** — `common/permissions.ts` gagne 4 nouvelles constantes :

- `CAN_MANAGE_USERS_SCOPED_WRITE` (DAF/RESPONSABLE_BU/RESPONSABLE_POLE) — nouvelle route `PATCH /users/:id/scoped` (`users.service.ts::updateScoped`) : un manager scopé corrige nom/e-mail/mot de passe et/ou planning des utilisateurs de son périmètre, jamais rôle/BU/pôle/manager, jamais un pair/supérieur, jamais lui-même. `setActive` étendu à DAF/RESPONSABLE_BU avec la même vérification.
- `CAN_MANAGE_HOLIDAYS` (CTO_ADMIN + DAF) — la DAF gère désormais les jours fériés (auparavant verrouillé sur `CAN_MANAGE_SETTINGS`, qui mélangeait branding technique et calendrier RH/paie).
- `CAN_MANAGE_ANNOUNCEMENTS_BU_SCOPE` (DAF/RESPONSABLE_BU) — annonces limitées à sa propre BU (jamais globales), avec vue de gestion distincte de la vue widget.
- `CAN_MANAGE_SCHEDULE_GROUPS_BU_SCOPE` (RESPONSABLE_BU) — groupes horaires limités à sa propre BU (débloque directement le cas du modèle nuit/week-end du Pôle TV/Radio, en attente depuis plusieurs sessions).
- PDG : nouvel accès lecture seule à l'organigramme via un composant dédié `ParametresReadOnly.tsx` (jamais de partage de code avec `ParametresClient.tsx`, pour ne prendre aucun risque qu'un contrôle de mutation fuite vers ce rôle).
- 29 nouveaux tests Jest (`users.service.spec.ts`, `announcements.service.spec.ts`, `presence.schedule-groups.spec.ts`) verrouillent ce scoping.
- **Non fait** : le câblage frontend de `/utilisateurs`, `/annonces`, `/presences/planning` pour que DAF/RESPONSABLE_BU/RESPONSABLE_POLE utilisent réellement ces nouveaux droits — seuls les endpoints API existent pour l'instant, l'UI de ces pages reste celle pensée pour leurs anciens droits plus restreints.

**Fiabilité backend** : `/health` interroge réellement PostgreSQL (503 si injoignable, au lieu de toujours 200) ; filtre d'exception global (`AllExceptionsFilter`) qui journalise systématiquement les erreurs 5xx avec un id de corrélation (avant : 2 fichiers sur 87 utilisaient `Logger`) ; validation des variables d'environnement au démarrage (`DATABASE_URL`/`JWT_SECRET` requis, `JWT_SECRET` ≥ 32 caractères, fail-fast sans nouvelle dépendance) ; rate-limit dédié (10/min) sur les 10 routes d'export PDF/Excel (Puppeteer/ExcelJS) ; `migration_lock.toml` régénéré (absent jusqu'ici) ; `docker-compose.yml` n'accepte plus de mot de passe Postgres par défaut faible (échoue explicitement si `POSTGRES_PASSWORD` non défini).

**CI** : premier pipeline GitHub Actions du projet (`.github/workflows/ci.yml`, à la racine du dépôt Git) — Postgres éphémère, `prisma migrate deploy`, `prettier --check`, type-check API+Web, `jest`, builds, `npm audit` informatif.

**Baseline Prisma corrigée** : question posée explicitement à l'utilisateur (le squash avait été refusé le 2026-08-03) — validée cette session compte tenu du nouveau contexte (déploiement OVH toujours "TODO", donc aucune base de production à réconcilier). Les 9 anciens dossiers de migrations (les deux baselines concurrentes `0004_module4_tabs`/`20260630000001_module3_presence`, qui créaient chacune un jeu complet de tables dont `business_units` en double, + les 7 migrations incrémentales appliquées jusqu'ici uniquement via `db push`) sont remplacés par une unique migration `20260824000000_init_baseline`, générée par `prisma migrate diff --from-empty --to-schema-datamodel schema.prisma` (sans connexion DB requise) — vérifiée par relecture (28 tables, 10 enums, tous les champs récents présents). La suppression des anciens dossiers a nécessité une confirmation explicite supplémentaire (le classifieur de permissions bloque par défaut les suppressions de fichiers versionnés). Le `continue-on-error` de l'étape `migrate deploy` dans la CI a été retiré en conséquence. **Non fait** : `prisma migrate deploy` non exécuté contre une vraie base neuve dans cet environnement (pas d'accès Docker/PostgreSQL) — à surveiller au premier run de la CI.

**Frontend** : `ParametresClient.tsx` utilise désormais `lib/http.ts` (`apiFetch`) au lieu d'un `fetch()` local qui ne redirigeait jamais vers `/login` sur une session expirée ; `Modal.tsx` gagne un focus initial + piège de focus (aucune gestion de focus n'existait) ; `DataTable.tsx` : en-têtes triables accessibles au clavier (`role="button"`, `tabIndex`, `aria-sort`) et bouton d'effacement de recherche avec `aria-label` ; `PlanningCalendar.tsx` : case de calendrier restructurée en `<div role="button">` contenant un vrai `<button>` (au lieu d'un `<span role="button">` imbriqué dans un `<button>`, invalide en HTML et inutilisable au clavier) ; `manifest.ts` : type MIME de l'icône personnalisée détecté depuis le contenu réel au lieu d'être toujours codé en dur `image/jpeg`.

- **Non fait** (liste complète dans `TACHE.md`) : découpage des plus gros composants, centralisation des listes de rôles dupliquées côté frontend, ESLint (API+Web), tests frontend, React Query/SWR, consolidation couleur de marque, `next/image`, mise à jour du service worker.
...
### Nouvelle demande réalisée — Sélecteur "employé CONGE existant" à la création d'un compte (2026-08-24)

- Demande : à la création d'un employé dans `/utilisateurs`, pouvoir choisir parmi les employés déjà connus de la plateforme CONGE, pour garantir que c'est bien le même employé des deux côtés (au lieu d'une saisie manuelle du matricule/identité pouvant diverger). Décision actée avec l'utilisateur (question à choix posée avant implémentation) : sélection optionnelle depuis CONGE dans le formulaire — ni validation stricte bloquante, ni provisionnement automatique CONGE ← VdM Intranet.
- `leave-sync.service.ts` gagne `getEmployees()` (appelle `GET {CONGE_API_URL}/api/employees`, même en-tête `x-intranet-secret` que l'endpoint congés, cache 5 min, dégradation silencieuse `[]`) et `isConfigured()`. `leave-match.util.ts::matchLeaveToUser` généralisé pour accepter toute identité `{matricule, email}` (plus seulement `ActiveLeave`), réutilisé pour le rapprochement employés. `leaves.service.ts::getCongeEmployeeCandidates()` ne renvoie que les employés CONGE sans compte VdM Intranet correspondant déjà (pas de doublons dans le sélecteur). Nouvelle route `GET /leaves/conge-employees`, réservée à `CAN_MANAGE_USERS` (CTO_ADMIN, PDG).
- `UsersManager.tsx` : formulaire "Nouvel utilisateur" (jamais en édition) — menu déroulant optionnel listant ces candidats, pré-remplit identifiant(=matricule)/prénom/nom/email ; champs modifiables ensuite.
- Tests réels : `npm run build:api` OK, `npm run build:web` OK, `npx jest` (`presence.service`, `presence.schedule-groups`, `reports.service`, `users.service`) 59/59 OK, `npx prettier --write` sur les fichiers modifiés OK.
- **Important pour le déploiement — CONGE est déjà en ligne** (`CONGE_API_URL` de prod visée dans `.env.example` : `https://rhvedem.veilleurdesmedias.org`), contrairement au reste de l'intégration Congés qui avait été développée avant toute mise en production. Voir la checklist détaillée ajoutée dans `TACHE.md` et `README.md` (section Déploiement) : nouvel endpoint `GET /api/employees` à créer et déployer côté dépôt VEDEM/CONGE (séparé, non présent ici), et `CONGE_API_URL`/`CONGE_API_SECRET` à renseigner dans le `.env` de production de vdm-intranet (actuellement vides même en local). Tant que ces deux points ne sont pas faits, le sélecteur reste invisible en production (dégradation silencieuse par design, la création manuelle continue de fonctionner).
- Contexte détaillé et contrat d'intégration consignés en mémoire (`conge-employees-integration.md`) pour les futures sessions.
- `TACHE.md` mis à jour avec le détail complet de cette session.

### Audit prod-readiness & préparation au déploiement (2026-08-28)

- Demande : analyser l'intégralité du dépôt pour juger s'il est prêt pour la prod, puis (suite immédiate, même journée) corriger tous les points relevés et préparer le repo au déploiement — les valeurs réelles de production seront renseignées directement sur le VPS avant la mise en ligne.
- Contrairement aux sessions précédentes, cet environnement (Windows) dispose de Node/npm/Docker : l'audit et les corrections ont donc pu être **vérifiés par exécution réelle** plutôt que par lecture de la documentation existante.
- **Bug bloquant trouvé et corrigé** : les fichiers `.tsbuildinfo` (cache incrémental TypeScript) étaient committés dans git. Un cache périmé faisait que `nest build` ne générait plus `dist/main.js`/`dist/app.module.js` (seuls les `.d.ts` étaient régénérés) → `node dist/main` (`start:prod`, ce que PM2 lance en prod) plantait avec `MODULE_NOT_FOUND`. C'est exactement le symptôme que la session du 2026-07-29 avait attribué à tort à _« une particularité de l'environnement »_ — reproduit, root-caused et corrigé : fichiers retirés du suivi git, `*.tsbuildinfo` ajouté à `.gitignore`, build propre vérifié.
- **Deuxième bug trouvé et corrigé** : `NEXT_PUBLIC_API_URL`/`NEXT_PUBLIC_APP_URL` n'étaient jamais injectées au build de production — Next.js ne charge un `.env` que depuis son propre dossier (`apps/web/`), jamais le `.env` racine (seul fichier `.env` du repo). En prod, le build serait retombé silencieusement sur `http://localhost:3001`, inutilisable pour un vrai visiteur (masqué en dev par coïncidence de valeurs). Nouveau `scripts/copy-web-env.js` (liste blanche `NEXT_PUBLIC_*` + `COOKIE_NAME`, jamais de secret) génère `apps/web/.env.production.local` depuis le `.env` racine, déclenché automatiquement via le hook npm `prebuild:web` avant `build:web` (même pattern que `predev:api` déjà en place). Vérifié dans la sortie réelle de `next build`.
- **Déploiement** : nouveau `ecosystem.config.js` (PM2, 2 process API+Web, API en instance unique — état en mémoire non partageable entre workers sans Redis) ; nouveau `deployment/nginx/vdm-intranet.conf` (reverse proxy 2 sous-domaines + upgrade WebSocket pour Socket.IO + timeout étendu exports PDF/Excel) ; `README.md` section "Déploiement OVH" réécrite avec les commandes réelles.
- **Hygiène** : `.gitattributes` ajouté (`eol=lf`, protège contre un futur commit CRLF accidentel qui casserait `prettier --check` en CI) ; `npm audit fix` non-breaking exécuté (21→20 vulnérabilités `--omit=dev`). Vulnérabilité critique restante (`tar` via `bcrypt→@mapbox/node-pre-gyp`) volontairement non forcée : testée via `overrides`, npm refuse de résoudre proprement, et le risque réel est quasi nul (extraction d'un binaire depuis une URL fixe, jamais d'entrée utilisateur) contre un risque concret de casser la compilation native de `bcrypt` sur le VPS — documenté dans `TACHE.md` plutôt que forcé aveuglément.
- **Validation de bout en bout réellement exécutée** (pas seulement lue) : `prisma migrate deploy` sur une vraie base PostgreSQL 16 neuve (conteneur Docker jetable) → les 3 migrations de la baseline squashée du 2026-08-24 s'appliquent proprement (jamais vérifié depuis cette date faute d'accès DB) ; seed exécuté sur cette base ; `node dist/main.js` démarré en `NODE_ENV=production` contre cette base → `/api/health` (200, vraie vérification DB) et `/api/auth/login` (200) fonctionnels. `tsc`/`jest` (102/102)/builds/`prettier --check` : tous OK.
